...
def main(driver,id,search_type):
    ...
    # 辞書が空(存在しないID)のときはFalse
    if not any(target_id_data[id]):
        print("SKIP!! Searching for a non-existent user.")
        with open(f"..\\data2\\{search_type}\\{id}.txt", 'w', encoding="utf-8") as f:
            for follow_name in set(): f.write(f"{follow_name}\n")
        return False
    if target_id_data[id][search_type] > 10000 or target_id_data[id][search_type] == 0:
        print("SKIP!! As the search target exceeds 200 or is 0.")
        with open(f"..\\data2\\{search_type}\\{id}.txt", 'w', encoding="utf-8") as f:
            for follow_name in set(): f.write(f"{follow_name}\n")
        return False
    if target_id_data[id]["LOCK"] == True:
        print("SKIP!! As the search target is a locked account.")
        with open(f"..\\data2\\{search_type}\\{id}.txt", 'w', encoding="utf-8") as f:
            for follow_name in set(): f.write(f"{follow_name}\n")
        return False
